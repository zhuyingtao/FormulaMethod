...
(* 
  natlist1_ind : forall P: natlist1 -> Prop,
                 P nnil1 ->
                 (forall (l:natlist) (n:nat), P l -> P nsnoc1 l n) ->
...
(* byntree_in : forall P: byntree -> Prop,
                P bempty ->
...
Definition P_comm' : nat -> nat -> Prop :=
  fun n m => n + m = m + n.

Theorem plus_comm''' : forall n m : nat,
  P_comm n m.
Proof.
  unfold P_comm. induction n. 
  Case "0". intros m. rewrite -> plus_0_r. reflexivity.
  Case "S". intros m. simpl. rewrite IHn. simpl. rewrite plus_n_Sm. reflexivity.
Qed.
(** [] *)
...
(** Given the following inductively defined proposition:
  ...
  no_longer_than_ind
       : forall (X : Set) (P : list X -> nat -> Prop),
          (forall n : nat, P (no_longer_that X [] n)) ->
