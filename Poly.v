...
(* 特征 ： 1. 做参数 2. 做返回值 3.存储在其他数据结构里（对象）
  嵌套函数
 *)
...
(* ###################################################### *)
(** ** Partial Application （部分 作用（函数调用））*)
...
(** In Coq, a function [f : A -> B -> C] really has the type [A
    ...
    This is called _uncurrying_.  With an uncurried binary function, 
    both arguments must be given at once as a pair; there is no partial 
    application. 

    Currying [A -> B -> C]        非Currying [(A *B) -> C]
*)
...
Eval compute in fold plus (map (fun n => n * n) gen_list 100).
l*)
...
(* When we want to perform an operation on (like a map) before aggregating.  *)
