...
Proof.
  intros. destruct (eq_id_dec x y) eqn:Y.
...
Proof.
  intros. apply update_neq. unfold not. intros. inversion H.
Qed.
(** [] *)
...
Proof.
  intros. unfold update. destruct (eq_id_dec x2 x1) eqn:Y.
...
Proof.
  intros. unfold update.  destruct (eq_id_dec x1 x2) eqn:Y.
  Case "x1 = x2". rewrite e in H. symmetry in H. assumption.
...
Proof.
  ...
  Case "X<-0". apply E_Ass. reflexivity.
  Case ";;". apply E_Seq with (st':= update (update empty_state X 0) Y 1).
  SCase "Y<-1". apply E_Ass. reflexivity.
  SCase "Z<-2". apply E_Ass. simpl. reflexivity.
Qed.
(** [] *)
...
match prog, stack with
  ...
  | (SLoad id) :: t, _=> s_execute st ((st id)::stack) t
  | (SPlus) :: t, (s1::(s2::s)) => s_execute st ((s2 + s1)::s) t
  | (SMinus) :: t, (s1::(s2::s)) => s_execute st ((s2 - s1)::s) t
  | (SMult) :: t, (s1::(s2::s)) => s_execute st ((s2 * s1)::s) t
  | [], s  => s                (* base *)
  |  _, [] => []                (* undefined *)
  |  _, _ => []                 (* undefined *)
end.
