...
Proof.
  intros p. destruct p as [x y]. simpl. reflexivity. Qed.
(** [] *)
...
Definition x : bintree :=
  node 1 (node 2 (empty) (empty)) 
         (node 3 (empty) (empty)).

Fixpoint treelength (x : bintree) : nat :=
  match x with
  | empty => 0
  | node n leftt rightt => 
      1 + treelength (leftt) + treelength (rightt)
  end.

Eval compute in (treelength x).
(* ================================================ *)
...
(*==================================================*)
Fixpoint pretree (x : bintree) : natlist :=
  match x with
  | empty => []
  | node n leftt rightt => 
    n :: (app (pretree leftt) (pretree rightt))
  end.

Eval compute in (pretree x).
(* ================================================= *)
