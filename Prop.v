...
(*  No - if we use induction on n, we end up having to consider ev (S n).
    We cannot determine whether S n is even or odd from the form given
    and have no further evidence for it. Furthermore, we have no mechanism
    for pulling out the S from ev (S n), and so we cannot apply the inductive
...
(* Once again, it tries to do induction over natural numbers, 
   instead of even natural numbers. *)
...
(* 
  inversion on E gives us some S (S n) to work with. If we destruct, 
  we only get some S n. We can strip the first S with pred, but we 
  will be left with an extra pred and have no way of reducing it to
  something over which we can apply evidence.
*)
...
(** Suppose we give Coq the following definition:
    ...
    - [R 2 [1,0]]
      -c1: R 0 [] -> c2 : R 1 [0] -> c2 : R 2 [1,0]
    - [R 1 [1,2,1,0]]
      -c1: R 0 [] -> c2 : R 1 [0] -> c2 : R 2 [1,0] -> c2 : R 3 [2,1,0] ->
       c3: R 2 [2,1,0] -> c3: R 1 [2,1,0] -> c2: R 2 [1,2,1,0] ->
       c3: R 1 [1,2,1,0]
    - [R 6 [3,2,1,0]]
      -not provable.
*)
